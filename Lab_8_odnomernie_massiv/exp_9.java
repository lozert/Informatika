...
public class exp_9 {
    public static void main(String args[]) {
        ...
        System.out.println("Введите значение порядка метрицы 2Nx2N ");
        int N = scn.nextInt();
        int n = 2 * N;
        int mas[][] = BigMas(n);
    ...
    public static int[][] BigMas(int n){
        ...
        for (int j = 0; j < MasLeft.length; j++) {/**Отвечает за левую часть массива**/
            for (int i = 0; i < MasLeft.length; i++) {
                mas[j][i] = MasLeft[j][i];
                mas[j + n / 2][i] = MasLeft[j][i];
            }
        }
        for (int j = 0; j < MasRight.length; j++) {/**Отвечает за правую большого массива**/
    ...
    public static int[][] MasForAll(int n, String m){
        int mas[][] = new int[n][n];
        int count = 0;
        if (m.equals("left")) {/**Заполение массива слевой половины**/
            while (mas[0][0] == 0) {/**Пока первый элемент равен 0**/
                for (int i = 1; i <= n; i++) {
                    int y = i;
                    for (int x = 1; x <= i; x++) {
                        count++;
        ...
        } else if (m.equals("right")) {/**Заполение массива правой половиня**/
            for (int i = 0; i <= n-2; i++) {
                int y = i+1;
                for (int x = 0; x < n - i; x++) {
                    if (mas[x][i] == 0) {
